...
const Shopfilter = ({ filterOption, filterState, setFilterState, clearFilter }) => {
  return (
    ...
    Clear Filters
      ...
      <div className="flex items-center space-x-2">
        ...
        {filterState.searchInput && (
          <button
            onClick={() => setFilterState({ ...filterState, searchInput: "" })}
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {filterOption.categories.map((cat) => (
            <label key={cat} className="capitalize cursor-pointer flex items-center space-x-2">
              <input
                type="radio"
                value={cat}
  ...
  <label key={pric.label} className="capitalize cursor-pointer flex items-center space-x-2">
    <input
      type="radio"
      name="price" 
      value={pric.label}
      checked={filterState.priceRange?.label === pric.label}
      onChange={() => setFilterState({ ...filterState, priceRange: pric })}
      className="accent-blue-500"
    />
  ...
  <div className="max-h-48 overflow-y-auto border scrollbar-hide p-2 rounded-lg shadow-sm bg-gray-50">
    <div className="flex flex-col space-y-2">
      {filterOption.brands.map((brand) => (
        <label key={brand} className="capitalize cursor-pointer flex items-center space-x-2">
          <input
            type="radio"
            value={brand}
            checked={filterState.brands === brand}
            onChange={(e) => setFilterState({ ...filterState, brands: e.target.value })}
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {filterOption.ratings.map((rate) => (
            <label key={rate} className="cursor-pointer flex items-center space-x-2">
              <input
                type="radio"
                value={rate}
                checked={filterState.ratings === rate}
                onChange={(e) => setFilterState({ ...filterState, ratings: e.target.value })}
                className=""
              />
