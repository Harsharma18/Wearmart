...
function Ordersummary() {
  ...
  const { totalItems, totalPrice, tax, taxRate, grandTotal } = useSelector(
    (state) => state.cart
  );
  ...
  const makePayment = async (e) => {
    ...
    const response = await fetch(
      "http://localhost:8080/api/order/create-checkout-session",
      {
        method: "POST",
        headers: header,
      ...
      <div className="px-6 py-4 space-y-5">
        ...
        <div className="px-4 mb-6">
          <button
            ...
            className="bg-red-500 px-3 py-1.5 text-white mt-2 rounded-md flex items-center justify-center mb-4"
          >
            Clear Cart{" "}
            <span>
          ...
          <button
            ...
            className="bg-green-700 px-3 py-1.5 text-white mt-2 rounded-md flex items-center justify-center mb-4"
          >
            Proceed Checkout{" "}
            <span className="ml-2">
