...
router.get("/userstats/:email", async (req, res) => {
  ...
  try {
    ...
    const totalAmountPayment = totalPaymentresult.length>0?totalPaymentresult[0].totalamount:0;
    //get totalReviews 
    const totalReview = await  Reviewmodel.countDocuments({author:user._id});
      //total purchase product
      const productPurchase = await Ordermodel.distinct("items.itemId",{email:email});
...
router.get("/adminstats",async(req,res)=>{
    try{
        ...
        const totalEarning = totalEarningResult.length>0?totalEarning[0].totalEarning:0;
